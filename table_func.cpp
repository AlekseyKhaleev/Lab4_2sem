...
int CreateTable(char *argv[], std::fstream &table_out, int rows_request) {
    ...
    * Вызываемые модули: "structs.h", "definitions.h", "draw_func.h", "print_func.h", <fstream>
    * Описание алгоритма:
    ...
    * 5) Объявляется и инициализируется начальным значением 1 переменная row_index, соответствующая номеру первой
    ...
    * 8) Выделяется память для массива структур PC размером rows_request, указатель на память - ParkUpdate
    ...
    *      - с помощью метода ParkUpdate[i].set() осуществляется ввод значений очередной структуры
    *   (строки) в соответствующей строке таблицы.
    *      - инициализированная структура записывается в двоичный файл
    ...
    *******************************************************************************************************************/
    ...
    try {
        ...
        auto ParkUpdate = new PC[rows_request];    // выделение памятиh
        for (int i = 0; i < rows_request; i++) {
            if (i < MAX_ROWS)
                /* Если не достигнута последняя строка в отображаемой таблице */
            {
                ParkUpdate[i].set(i); // ввод данных в соответствующих ячейках
                ParkUpdate[i].write(table_out); // запись экземпляра класса в файл
            } else
                /* Если достигнута последняя строка в отображаемой таблице */
            {
                ...
                PrintTable(ParkUpdate, MAX_ROWS-1, row_index);
                /* Инициализация очередной структуры, курсор ввода в последней строке */
                ParkUpdate[i].set(MAX_ROWS - 1);
                ParkUpdate[i].write(table_out); // запись экземпляра класса в файл
            }
        }
        delete[] ParkUpdate; // освобождение памяти
...
void ViewTable(char *argv[], std::fstream &table_in, int rows_request, int row_index) {
    ...
    * 6) Выделяется память для массива структур PC размером rows_to_print, указатель на память - ParkContent
    ...
    *******************************************************************************************************************/
    ...
    try {
        ...
        auto *ParkContent = new PC[rows_to_print]; // выделение памяти
        for (int i = 0; i < rows_to_print; i++) {
            ParkContent[i].read(table_in); // чтение данных таблицы в массив экземпляров классов
        }
        GenerateTablePage(ParkContent, argv, rows_to_print, real_rows, row_index); // вывод основной страницы программы
            ...
            switch (UserKey()) { // считывание кода клавиши введенной пользователем в режиме "реального времени"
                ...
                case Up: {
                    if (row_index > 0) { // если таблица отображена не сначала
                        ...
                        GenerateTablePage(ParkContent, argv, rows_to_print, real_rows, row_index);
                    }
                    break;
                }
...
void GenerateTablePage(PC ParkContent[], char* argv[], int rows_to_print, int real_rows, int row_index){
    ...
    * Исходные данные:
    *      ParkContent   - массив структур, содержащий данные таблицы
    *      argv          - массив аргументов командной строки, с которыми была запущена программа
    ...
    *******************************************************************************************************************/
    ...
    PrintTable(ParkContent, rows_to_print, row_index); // Заполнение строк таблицы
